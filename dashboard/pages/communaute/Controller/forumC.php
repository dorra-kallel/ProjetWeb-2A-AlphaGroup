...
class forumC
{
 public function afficherforums()
    {  $sql= " SELECT * FROM forum ORDER BY date_post DESC" ; 
    ...
    {
        $sql="insert into forum (titre,description,image,comu,ad)
        values (:titre,:description,:image,:comu,:ad)";
        $db=config::getConnexion();
    ...
    function modifierforum($forum, $idforum){
        try {
            ...
            $query->bindValue(':ad',$forum->getad());
