...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
  ...
  public function rechercherInredients($name){
    try{
    $db=session_start();
    $stmt = $this->db->pdo->prepare('SELECT * FROM tbl_users where name like :name');
    $$stmt = $this->db->pdo->prepare->bindValue(':nom',"%".$nom."%");
    $stmt = $this->db->pdo->prepare->execute();
    return $stmt = $this->db->pdo->prepare->fetchAll();
...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
  ...
  public function selectAllUserData(){
    $sql = "SELECT * FROM tbl_users ORDER BY created_at DESC";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->execute();
